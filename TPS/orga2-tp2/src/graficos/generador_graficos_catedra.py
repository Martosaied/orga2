...
with PdfPages('fig3A.pdf') as pdf:
    ...
    ax.set_title('Comparacion de Optimizaciones')
    pdf.savefig()
    plt.close()
...
with PdfPages('fig4A.pdf') as pdf:
    ...
    plt.ylim((-2,10))
    plt.title('Busqueda del minimo parametro')
    plt.xlabel('Valor del parametro')
    plt.ylabel('Relacion entre A y B')
    pdf.savefig()
    plt.close()
...
with PdfPages('fig5A.pdf') as pdf:
    ...
    plt.title('Rendimiento segun tamano de la imagen')
    plt.xlabel('Tamano de la imagen')
    plt.ylabel('Cantidad de Ticks')
    pdf.savefig()
    plt.close()

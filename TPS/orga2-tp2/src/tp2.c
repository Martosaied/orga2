...
int main(int argc, char **argv)
{
    ...
    if (!config.nombre)
    {
        ...
        printf("  Implementación     : %s\n", config.tipo_filtro == FILTRO_C ? "C" : (config.tipo_filtro == FILTRO_ASM ? "ASM" : (config.tipo_filtro == 2 ? "ASM_V2" : "ASM_V3")));
    ...
    snprintf(config.archivo_salida, sizeof(config.archivo_salida), "%s/%s.%s.%s%s.bmp",
             ...
             config.nombre_filtro, config.tipo_filtro == FILTRO_C ? "C" : (config.tipo_filtro == FILTRO_ASM ? "ASM" : (config.tipo_filtro == 2 ? "ASM_V2" : "ASM_V3")), config.extra_archivo_salida);

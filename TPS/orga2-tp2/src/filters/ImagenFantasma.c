...
void ImagenFantasma_asm_v2(uint8_t *src, uint8_t *dst, int width, int height,
                           int src_row_size, int dst_row_size, int offsetx, int offsety);
void ImagenFantasma_asm_v3(uint8_t *src, uint8_t *dst, int width, int height,
                           int src_row_size, int dst_row_size, int offsetx, int offsety);
...
{
    ImagenFantasma_fn_t *ImagenFantasma;
    switch (config->tipo_filtro)
    {
    case 0:
        ImagenFantasma = ImagenFantasma_c;
        break;
    case 1:
        ImagenFantasma = ImagenFantasma_asm;
        break;
    case 2:
        ImagenFantasma = ImagenFantasma_asm_v2;
        break;
    case 3:
        ImagenFantasma = ImagenFantasma_asm_v3;
        break;
    }
    ...
    if (offsety < 0)
        offsety = 0;
    if (offsetx > info.width)
        offsetx = info.width / 2;
    if (offsety > info.height)
